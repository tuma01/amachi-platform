...
@Tag(name = "FamilyHistory", description = "Antecedentes patológicos familiares")
public interface FamilyHistoryApi extends GenericApi<FamilyHistoryDto> {
    String NAME_API = "FamilyHistory";

    @GetMapping(value = ID, produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Get " + NAME_API + " by ID", responses = {@ApiResponse(responseCode = "200"), @ApiResponse(responseCode = "404")})
    ResponseEntity<FamilyHistoryDto> getFamilyHistoryById(@PathVariable("id") @NonNull Long id);

    @PostMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Create " + NAME_API, responses = {@ApiResponse(responseCode = "201"), @ApiResponse(responseCode = "400")})
    ResponseEntity<FamilyHistoryDto> createFamilyHistory(@Valid @RequestBody @NonNull FamilyHistoryDto dto);

    @PutMapping(value = ID, produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Update " + NAME_API, responses = {@ApiResponse(responseCode = "200"), @ApiResponse(responseCode = "404")})
    ResponseEntity<FamilyHistoryDto> updateFamilyHistory(@PathVariable("id") @NonNull Long id, @Valid @RequestBody @NonNull FamilyHistoryDto dto);

    @DeleteMapping(value = ID)
    @Operation(summary = "Delete " + NAME_API, responses = {@ApiResponse(responseCode = "204"), @ApiResponse(responseCode = "404")})
    ResponseEntity<Void> deleteFamilyHistory(@PathVariable("id") @NonNull Long id);

    @GetMapping(value = ALL, produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Get all " + NAME_API)
    ResponseEntity<List<FamilyHistoryDto>> getAllFamilyHistories();

    @GetMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Get paginated " + NAME_API)
    ResponseEntity<PageResponseDto<FamilyHistoryDto>> getPaginatedFamilyHistories(
